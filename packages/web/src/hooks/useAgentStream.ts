...
import type { AgentSummary, ChatResponse, ToolEntry } from "../types"
...
export function useAgentStream(
      ...
      if (currentAgentRef.current) {
        setStreamState((prev) => {
          const summary: AgentSummary = {
            ...
            tools: [...prev.tools],
            durationMs: Date.now() - agentStartRef.current,
            startedAt: agentStartRef.current,
          }
          onAgentCompleteRef.current?.(summary)
          return {
            ...prev,
            completedAgents: [...prev.completedAgents, summary],
            tools: [],
            llmText: "",
            activeAgent: name,
          }
        ...
        setStreamState((prev) => ({ ...prev, activeAgent: name, tools: [], llmText: "" }))
      }
  ...
  const finalizeAgent = useCallback(() => {
    ...
    setStreamState((prev) => {
      ...
      onAgentCompleteRef.current?.(summary)
      return { ...prev, completedAgents: [...prev.completedAgents, summary], tools: [], llmText: "", activeAgent: null }
    })
        ...
        try {
          ...
          for await (const event of stream) {
            if (abortRef.current) {
              setStreamState((prev) => ({
                ...prev,
                error: "La conexion ha excedido el tiempo limite (5 min). Intenta de nuevo.",
              }))
              return
            }
                  ...
                  if (lastMsg.tool_calls?.length) {
                    ...
                    if (subagent) {
                      ...
                      if (newTools.length) {
                        ...
                        setStreamState((prev) => ({ ...prev, tools: [...prev.tools, ...newTools] }))
                      }
                    }
                  }
                }
                    ...
                    if (toolMsg.name) {
                      ...
                      setStreamState((prev) => ({
                        ...
                        tools: prev.tools.map((t) =>
                          t.name === toolMsg.name && t.status === "running" ? { ...t, status: newStatus } : t,
                        ),
                      }))
                    }
                  }
            ...
            if (ev.event === "messages/partial" && Array.isArray(ev.data) && ev.data.length > 0) {
              ...
              if (text) {
                setStreamState((prev) => ({ ...prev, llmText: text }))
              }
            }
