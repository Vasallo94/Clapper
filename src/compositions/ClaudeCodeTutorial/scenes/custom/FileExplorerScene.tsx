...
const FileTreeEntry: React.FC<{
...
}> = ({ file, index, expandFile, expandStart, tokens }) => {
  const frame = useCurrentFrame()
  const { fps } = useVideoConfig()

  const fileStagger = Math.ceil(fps * 0.5)
  const delay = index * fileStagger
  const s = spring({
  ...
  return (
    ...
    >
      ...
      <span>{file.name}</span>
      {file.isNew && (
        <span style={{ fontSize: 10, color: tokens.terminal.successColor, fontWeight: 700, marginLeft: 4 }}>NEW</span>
      )}
    </div>
  )
}
    ...
    >
      ...
      >
        ...
        >
          ...
          <span style={{ fontFamily: tokens.monoFontFamily, fontSize: 12, color: tokens.foreground }}>
            {expandFile}
          </span>
        </div>

        {/* Content area — line-by-line reveal is Phase 2 content animation */}
        <div
          style={{
            padding: "20px 24px",
            fontFamily: tokens.monoFontFamily,
